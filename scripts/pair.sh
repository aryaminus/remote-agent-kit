#!/usr/bin/env bash
# pair.sh — pair phones/clients with the live box.
#
#   ./scripts/pair.sh        # print the steps (URLs resolved for YOUR box)
#   ./scripts/pair.sh --qr   # do it all: SSH in, run pairing, stream the QR
#                            # right here — point the phone camera at YOUR screen
#
# Perch QR itself is generated by the public perch-site pair.sh ON THE SERVER
# (it reads the server's API key — which is why the key never crosses chat,
# logs, or git; only pixels on screens you own).
#
...
if [[ "${1:-}" == "--qr" ]]; then
  # No local Tailscale? Ask the box for its own serve URL instead.
  if [[ -z "$HTTPS_URL" ]]; then
    # shellcheck disable=SC2029
    HTTPS_URL="$($SSH python3 -c 'import json,subprocess
try:
    out = subprocess.run(["tailscale","status","--json"], capture_output=True, text=True, timeout=15).stdout
    print("https://" + json.loads(out).get("Self",{}).get("DNSName","").rstrip("."))
except Exception:
    print("")' 2>/dev/null || true)"
  fi
  [[ -n "$HTTPS_URL" && "$HTTPS_URL" != "https://" ]] || { echo "Cannot reach the box to derive its URL."; exit 1; }
  echo "Point your phone camera at THIS screen the moment the QR appears."
  echo "(Fetching the pairing code from the box — key never leaves it except as pixels.)"
  # NOTE: non-interactive SSH skips .bashrc, so hermes isn't on PATH by
  # default — export it explicitly (interactive logins get it from .bashrc).
  # shellcheck disable=SC2029  # expanded client-side on purpose (URL only, never the key)
  $SSH bash -c '"export PATH=\"$HOME/.local/bin:$PATH\"; curl -fsSL https://aryaminus.github.io/perch-site/pair.sh -o ~/pair.sh && bash ~/pair.sh --url '"$HTTPS_URL"'"'
  exit 0
fi
